...
export default function Pomodoro(){
  ...
  const [left,setLeft]=useState(0); // in secondi
  const [done,setDone]=useState(0); // focus completati
    ...
    ref.current = window.setInterval(()=>{
      setLeft(s=>{
        ...
        if(state==='focus'){
          ...
          setDone(nextDone);
          if(nextDone % cycles === 0){ setState('long'); return longMin*60; }
          setState('short'); return shortMin*60;
        } else if(state==='short'){
          setState('focus'); return focusMin*60;
        } else {
          setState('focus'); return focusMin*60;
        }
      });
