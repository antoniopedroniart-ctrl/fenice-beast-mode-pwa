...
function fmtItem(name:string, qty:number, unit:string){
  return (unit==='g' || unit==='ml')
    ? `${name} ${round5(qty)}${unit}`
    : `${name} ${Math.max(1, Math.round(qty))}`;
}
...
function calcMacros(phase:Phase, weight:number, bf:number): Macros {
  ...
  const F = 0.8 * lbm;
  const tdee = 34 * weight; // attivo (≥10k passi)
  let calories = tdee;
...
export default function NutritionPlanner(){
  ...
  function scale(rec:Rec, a:number){
    const items = rec.items.map(it => {
      const { name, qty, unit } = parseItem(it);
      return fmtItem(name, qty*a, unit);
    });
    return { kcal:rec.kcal*a, P:rec.P*a, C:rec.C*a, F:rec.F*a, items, name:rec.name };
  }

  function bestFor(t:{k:number;p:number;c:number;f:number}){
    let best:any=null, bestErr=Number.POSITIVE_INFINITY;
    const score=(K:number,P:number,C:number,F:number)=>
      Math.abs(K-t.k)/Math.max(150,t.k)*2 +
      Math.abs(P-t.p)/Math.max(10,t.p)*2 +
      Math.abs(C-t.c)/Math.max(20,t.c) +
      Math.abs(F-t.f)/Math.max(10,t.f);

    for(const r of RECIPES){
      const a = Math.min(1.8, Math.max(0.5, t.k / r.kcal));
      const s = scale(r, a);
      const e = score(s.kcal, s.P, s.C, s.F);
      if (e < bestErr){ bestErr=e; best=s; }
    }
    return best;
  }

  const meals = useMemo(()=> targets.map(t => bestFor(t)), [targets]);

  // Export / Rigenera tramite CustomEvent (bottoni nella pagina Nutrition)
  useEffect(()=>{
    async function onExport(){
      try{
        ...
        alert('Export PDF non disponibile in questo ambiente.');
      }
    }
  ...
  }, [settings.plan, macros.calories, macros.P, macros.C, macros.F]);
  ...
  return (
    <div ref={sectionRef}>
      ...
      <div className="muted" style={{marginTop:8}}>Le grammature sono scalate automaticamente per matchare i macro dello slot.</div>
    </div>
  );
}
