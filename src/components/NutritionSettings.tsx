...
/** Caricamento sicuro dal localStorage */
...
export default function NutritionSettings() {
  ...
  // ✅ niente "}||{ ... }" fuori dalla callback; fallback gestito DENTRO.
  const [state, setState] = useState<Settings>(() => {
    const saved = safeLoad<Settings>(KEY);
    ...
    <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(4, minmax(0, 1fr))' }}>
      <div>
        ...
        <select
          className="input"
          value={state.phase}
          onChange={(e) => setState({ ...state, phase: e.target.value as Phase })}
        >
      ...
      <div>
        ...
        <select
          className="input"
          value={state.plan}
          onChange={(e) => setState({ ...state, plan: e.target.value as Plan })}
        >
      ...
      <div>
        ...
        <select
          className="input"
          value={String(state.meals)}
          onChange={(e) => setState({ ...state, meals: Number(e.target.value) })}
        >
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <input
            ...
            type="number"
            value={state.bf}
            onChange={(e) => setState({ ...state, bf: Number(e.target.value) })}
          />
        </div>
      </div>
    </div>
