...
export default function NutritionSettings(){
  const [state,setState]=useState(()=>{ try{return JSON.parse(localStorage.getItem(KEY)||'')}catch{return null} }||{
    phase:'recomp' as Phase, plan:'base4' as Plan, meals:4, weight:80.35, bf:12.5
  });
    ...
    <div style={{display:'grid', gap:10, gridTemplateColumns:'repeat(4,minmax(0,1fr))'}}>
      ...
      <div><div className="muted">Schema pasti</div>
        <select className="input" value={state.plan} onChange={e=>setState({...state, plan:e.target.value as Plan})}>
          <option value="base4">4 pasti</option><option value="inter5">5 pasti</option><option value="fast2">Fast-Track (2-3)</option>
        </select>
      </div>
